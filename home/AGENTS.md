...
- **Brief the reviewer, every time, unprompted.** It reads `git diff master...HEAD` and never this conversation, so context I don't send is context lost. Immediately after any `/code-review` launch, `SendMessage` its `agentId` (from the `forked-skill-launch` line; background agents are reachable that way) with what the diff can't show: the **verification boundary** — what was actually executed vs not, so it neither reports the untested path as a gap nor assumes it away; **where the risk concentrates**; **known-and-accepted trade-offs**, so it doesn't spend budget re-deriving them; and any **cross-PR interaction**. Always tell it not to let this narrow its search and to contradict me if the code disagrees — briefing must not steer conclusions. Skip the send only when there is genuinely nothing beyond the diff worth saying; boilerplate is noise. Don't wait to be asked (agreed 2026-07-26 — you will forget, and I own it).
